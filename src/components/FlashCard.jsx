...
export function FlashCard({ productCode, value, setEntries, user }) {
	async function handleDelete() {
		try {
			const docRef = doc(db, 'users', user.uid, 'productCode', productCode);
			await deleteDoc(docRef);
			setEntries((prev) => prev.filter((entry) => entry.key !== productCode));
		} catch (error) {
